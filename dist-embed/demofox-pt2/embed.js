...
var ce = (n, e, t) => e in n ? ie(n, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : n[e] = t;
var h = (n, e, t) => ce(n, typeof e != "symbol" ? e + "" : e, t);
function Y(n, e, t) {
...
function le(n, e, t) {
  const r = Y(n, n.VERTEX_SHADER, e), s = Y(n, n.FRAGMENT_SHADER, t), i = n.createProgram();
  if (!i)
    throw new Error("Failed to create program object");
  if (n.attachShader(i, r), n.attachShader(i, s), n.linkProgram(i), !n.getProgramParameter(i, n.LINK_STATUS)) {
    const c = n.getProgramInfoLog(i);
    throw n.deleteProgram(i), n.deleteShader(r), n.deleteShader(s), new Error(`Program linking failed:
${c}`);
  }
  return n.detachShader(i, r), n.detachShader(i, s), n.deleteShader(r), n.deleteShader(s), i;
}
function ue(n) {
...
function Q(n, e) {
  ...
  if (r !== n.FRAMEBUFFER_COMPLETE)
    throw n.deleteFramebuffer(t), new Error(`Framebuffer incomplete: ${Ae(n, r)}`);
...
function pe(n, e, t, r) {
  ...
  for (let c = 0; c < 256; c++) {
    const d = t.get(c) || !1, P = r.get(c) || 0, x = (0 * 256 + c) * 4;
    p[x + 0] = d ? 1 : 0, p[x + 1] = d ? 1 : 0, p[x + 2] = d ? 1 : 0, p[x + 3] = 1;
    const I = (2 * 256 + c) * 4;
    p[I + 0] = P, p[I + 1] = P, p[I + 2] = P, p[I + 3] = 1;
  }
...
class he {
  constructor(e) {
    ...
    this.gl = e.gl, this.project = e.project, this._width = this.gl.drawingBufferWidth, this._height = this.gl.drawingBufferHeight, this.initExtensions(), this._blackTexture = fe(this.gl);
    const t = ge(this.gl);
  ...
  step(e, t) {
    ...
    const i = [this._width, this._height, 1], p = this._time, c = s, d = this._frame, P = t;
    r.viewport(0, 0, this._width, this._height);
    const x = ["BufferA", "BufferB", "BufferC", "BufferD", "Image"];
    for (const I of x) {
      const S = this._passes.find((j) => j.name === I);
  ...
  resize(e, t) {
    ...
    for (const s of this._passes)
      r.deleteTexture(s.currentTexture), r.deleteTexture(s.previousTexture), r.deleteFramebuffer(s.framebuffer), s.currentTexture = K(r, e, t), s.previousTexture = K(r, e, t), s.framebuffer = Q(r, s.currentTexture);
  }
  /**
  ...
  initProjectTextures() {
    ...
    for (const t of this.project.textures) {
      ...
      i.crossOrigin = "anonymous", i.onload = () => {
        ...
        e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_S, c), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_T, c), t.filter === "linear" && e.generateMipmap(e.TEXTURE_2D), s.width = i.width, s.height = i.height, console.log(`Loaded texture '${t.name}': ${i.width}x${i.height}`);
      }, i.onerror = () => {
   ...
   */
  initRuntimePasses() {
    const e = this.gl, t = this.project, r = ue(e), s = ["BufferA", "BufferB", "BufferC", "BufferD", "Image"];
    for (const i of s) {
      const p = t.passes[i];
      if (!p) continue;
      ...
      try {
        const d = le(e, de, c), P = {
          program: d,
          iResolution: e.getUniformLocation(d, "iResolution"),
          iTime: e.getUniformLocation(d, "iTime"),
          iTimeDelta: e.getUniformLocation(d, "iTimeDelta"),
          iFrame: e.getUniformLocation(d, "iFrame"),
          iMouse: e.getUniformLocation(d, "iMouse"),
          iChannel: [
            e.getUniformLocation(d, "iChannel0"),
            e.getUniformLocation(d, "iChannel1"),
            e.getUniformLocation(d, "iChannel2"),
            e.getUniformLocation(d, "iChannel3")
          ]
        }, x = K(e, this._width, this._height), I = K(e, this._width, this._height), S = Q(e, x), j = {
          name: i,
        ...
        const P = d instanceof Error ? d.message : String(d), x = this.getLineMapping(), I = P.match(/ERROR:\s*\d+:(\d+):/);
        let S = !1, j = null;
        if (I && this.project.commonSource) {
          const k = parseInt(I[1], 10), m = x.boilerplateLinesBeforeCommon + 2, y = m + x.commonLineCount - 1;
          k >= m && k <= y && (S = !0, j = k - m + 1);
        }
...
`);
  ...
  preprocessCubemapTextures(e) {
    ...
    return e.replace(t, (r, s, i) => i.includes("fragCoord") || // Using fragCoord directly
    i.includes("/") || // Division (likely uv calculation)
    /\.xy\s*$/.test(i.trim()) || // Ends with .xy swizzle
    /\.st\s*$/.test(i.trim()) || // Ends with .st swizzle
    /^vec2\s*\(/.test(i.trim()) ? r : `texture(${s}, _st_dirToEquirect(${i}))`);
  }
  // ===========================================================================
  // Pass Execution
  // ===========================================================================
  ...
  bindChannelTextures(e) {
    const t = this.gl;
    for (let r = 0; r < 4; r++) {
      const s = e.projectChannels[r], i = this.resolveChannelTexture(s, e.name);
  ...
  resolveChannelTexture(e, t) {
    switch (e.kind) {
      ...
      case "texture2D": {
        const s = this._textures.find((i) => i.name === e.name);
        if (!s)
...
class Ie {
  constructor(e) {
    ...
    this.gl = t, this.updateCanvasSize(), this.engine = new he({
      gl: this.gl,
      project: e.project
   ...
   */
  presentToScreen() {
    const e = this.gl, t = this.engine._passes.filter((s) => s.name === "Image");
    if (t.length === 0) {
  ...
  setupMouseTracking() {
    const e = (r) => {
      const s = this.canvas.getBoundingClientRect(), i = (r.clientX - s.left) * this.pixelRatio, p = (s.height - (r.clientY - s.top)) * this.pixelRatio;
  ...
  showErrorOverlay(e) {
    ...
    const t = e.filter((P) => P.isFromCommon), r = e.filter((P) => !P.isFromCommon), c = [...t.length > 0 ? [t[0]] : [], ...r].map(({ passName: P, error: x, source: I, isFromCommon: S, originalLine: j }) => {
      const k = x.replace(`Shader compilation failed:
`, "");
      let m = k;
      return S && j !== null && (m = k.replace(/Line \d+:/, `Line ${j}:`), m = m.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${j}:`)), {
        passName: S ? "common.glsl" : P,
        error: this.parseShaderError(m),
        codeContext: S ? this.extractCodeContextFromCommon(j) : this.extractCodeContext(m, I)
      };
    }).map(({ passName: P, error: x, codeContext: I }) => `
      <div class="error-section">
        <div class="error-pass-name">${P}</div>
        <pre class="error-content">${this.escapeHTML(x)}</pre>
        ${I ? `<pre class="error-code-context">${I}</pre>` : ""}
      </div>
    `).join("");
    ...
    `;
    const d = this.errorOverlay.querySelector(".error-close");
    d && d.addEventListener("click", () => this.hideErrorOverlay());
  }
  /**
...
`);
  ...
  extractCodeContext(e, t) {
    ...
    if (!r) return null;
    const s = parseInt(r[1], 10), i = t.split(`
`), p = 3, c = Math.max(0, s - p - 1), d = Math.min(i.length, s + p);
    return i.slice(c, d).map((I, S) => {
      const j = c + S + 1, k = j === s, m = String(j).padStart(4, " "), y = this.escapeHTML(I);
      return k ? `<span class="error-line-highlight">${m} │ ${y}</span>` : `<span class="context-line">${m} │ ${y}</span>`;
    }).join("");
  }
  /**
  ...
  extractCodeContextFromCommon(e) {
    ...
    if (!t) return null;
    const r = t.split(`
`), s = 3, i = Math.max(0, e - s - 1), p = Math.min(r.length, e + s);
    return r.slice(i, p).map((P, x) => {
      const I = i + x + 1, S = I === e, j = String(I).padStart(4, " "), k = this.escapeHTML(P);
      return S ? `<span class="error-line-highlight">${j} │ ${k}</span>` : `<span class="context-line">${j} │ ${k}</span>`;
    }).join("");
  }
  /**
...
(function(n) {
   ...
   */
  var t = function(r) {
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, i = 0, p = {}, c = {
      /**
      ...
      util: {
        encode: function o(a) {
          return a instanceof d ? new d(a.type, o(a.content), a.alias) : Array.isArray(a) ? a.map(o) : a.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/\u00a0/g, " ");
        },
        /**
         ...
         */
        objId: function(o) {
          return o.__id || Object.defineProperty(o, "__id", { value: ++i }), o.__id;
        },
        /**
         ...
         * @template T
         */
        clone: function o(a, l) {
          l = l || {};
          var u, f;
          switch (c.util.type(a)) {
            case "Object":
              if (f = c.util.objId(a), l[f])
                return l[f];
              u = /** @type {Record<string, any>} */
              {}, l[f] = u;
              for (var A in a)
                a.hasOwnProperty(A) && (u[A] = o(a[A], l));
              return (
                /** @type {any} */
                u
              );
            case "Array":
              return f = c.util.objId(a), l[f] ? l[f] : (u = [], l[f] = u, /** @type {Array} */
              /** @type {any} */
         ...
         * @returns {void}
         */
        setLanguage: function(o, a) {
          o.className = o.className.replace(RegExp(s, "gi"), ""), o.classList.add("language-" + a);
        },
        /**
      ...
      languages: {
         ...
         */
        extend: function(o, a) {
          var l = c.util.clone(c.languages[o]);
          for (var u in a)
            l[u] = a[u];
          return l;
        },
        ...
        insertBefore: function(o, a, l, u) {
          ...
          for (var C in f)
            if (f.hasOwnProperty(C)) {
              if (C == a)
                for (var g in l)
                  l.hasOwnProperty(g) && (A[g] = l[g]);
              l.hasOwnProperty(C) || (A[C] = f[C]);
            }
          var T = u[o];
          return u[o] = A, c.languages.DFS(c.languages, function(q, H) {
            H === T && q != o && (this[q] = A);
          }), A;
        },
        // Traverse a language definition with Depth First Search
        DFS: function o(a, l, u, f) {
          f = f || {};
          var A = c.util.objId;
          for (var C in a)
            if (a.hasOwnProperty(C)) {
              ...
              T === "Object" && !f[A(g)] ? (f[A(g)] = !0, o(g, l, null, f)) : T === "Array" && !f[A(g)] && (f[A(g)] = !0, o(g, l, C, f));
            }
        }
      },
      plugins: {},
       ...
       * @public
       */
      highlightAll: function(o, a) {
        c.highlightAllUnder(document, o, a);
      },
      /**
       ...
       * @public
       */
      highlightAllUnder: function(o, a, l) {
        var u = {
          callback: l,
          container: o,
        ...
        for (var f = 0, A; A = u.elements[f++]; )
          c.highlightElement(A, a === !0, u.callback);
      },
      /**
       ...
       * @public
       */
      highlightElement: function(o, a, l) {
        var u = c.util.getLanguage(o), f = c.languages[u];
        c.util.setLanguage(o, u);
        var A = o.parentElement;
        A && A.nodeName.toLowerCase() === "pre" && c.util.setLanguage(A, u);
        var C = o.textContent, g = {
          element: o,
          language: u,
          grammar: f,
          code: C
        ...
        if (c.hooks.run("before-sanity-check", g), A = g.element.parentElement, A && A.nodeName.toLowerCase() === "pre" && !A.hasAttribute("tabindex") && A.setAttribute("tabindex", "0"), !g.code) {
      ...
      highlight: function(o, a, l) {
        ...
        return u.tokens = c.tokenize(u.code, u.grammar), c.hooks.run("after-tokenize", u), d.stringify(c.util.encode(u.tokens), u.language);
      },
      /**
      ...
      tokenize: function(o, a) {
        ...
        var f = new I();
        return S(f, f.head, o), x(o, f, a, f.head, 0), k(f);
      },
      /**
       * @namespace
       * @memberof Prism
    ...
    r.Prism = c;
    function d(o, a, l, u) {
      this.type = o, this.content = a, this.alias = l, this.length = (u || "").length | 0;
    }
    d.stringify = function o(a, l) {
      if (typeof a == "string")
        return a;
      if (Array.isArray(a)) {
        var u = "";
      ...
      var f = {
        type: a.type,
        content: o(a.content, l),
        tag: "span",
        classes: ["token", a.type],
        attributes: {},
        language: l
      }, A = a.alias;
      A && (Array.isArray(A) ? Array.prototype.push.apply(f.classes, A) : f.classes.push(A)), c.hooks.run("wrap", f);
      var C = "";
      for (var g in f.attributes)
        C += " " + g + '="' + (f.attributes[g] || "").replace(/"/g, "&quot;") + '"';
      return "<" + f.tag + ' class="' + f.classes.join(" ") + '"' + C + ">" + f.content + "</" + f.tag + ">";
    };
    function P(o, a, l, u) {
      o.lastIndex = a;
      var f = o.exec(l);
      ...
      for (var C in l)
        if (!(!l.hasOwnProperty(C) || !l[C])) {
          ...
          for (var T = 0; T < g.length; ++T) {
            if (A && A.cause == C + "," + T)
              return;
            var q = g[T], H = q.inside, w = !!q.lookbehind, W = !!q.greedy, N = q.alias;
            if (W && !q.pattern.global) {
              var ae = q.pattern.toString().match(/[imsuy]*$/)[0];
              q.pattern = RegExp(q.pattern.source, ae + "g");
            }
            for (var z = q.pattern || q, O = u.next, U = f; O !== a.tail && !(A && U >= A.reach); U += O.value.length, O = O.next) {
              var v = O.value;
              ...
              if (!(v instanceof d)) {
                var F = 1, V;
                if (W) {
                  if (V = P(z, U, o, w), !V || V.index >= o.length)
                    break;
                  var G = V.index, oe = V.index + V[0].length, b = U;
                  for (b += O.value.length; G >= b; )
                    O = O.next, b += O.value.length;
                  if (b -= O.value.length, U = b, O.value instanceof d)
                    continue;
                  for (var J = O; J !== a.tail && (b < oe || typeof J.value == "string"); J = J.next)
                    F++, b += J.value.length;
                  F--, v = o.slice(U, b), V.index -= U;
                } else if (V = P(z, 0, v, w), !V)
                  continue;
                var G = V.index, D = V[0], M = v.slice(0, G), X = v.slice(G + D.length), B = U + v.length;
                A && B > A.reach && (A.reach = B);
                var E = O.prev;
                M && (E = S(a, E, M), U += M.length), j(a, E, F);
                var se = new d(C, H ? c.tokenize(D, H) : D, N, D);
                if (O = S(a, E, se), X && S(a, O, X), F > 1) {
                  var Z = {
                    cause: C + "," + T,
                    reach: B
                  };
                  x(o, a, l, O.prev, U, Z), A && Z.reach > A.reach && (A.reach = Z.reach);
                }
              }
            }
          }
    ...
    function I() {
      var o = { value: null, prev: null, next: null }, a = { value: null, prev: o, next: null };
      o.next = a, this.head = o, this.tail = a, this.length = 0;
    }
    function S(o, a, l) {
      var u = a.next, f = { value: l, prev: a, next: u };
      return a.next = f, u.prev = f, o.length++, f;
    }
    function j(o, a, l) {
      for (var u = a.next, f = 0; f < l && u !== o.tail; f++)
        u = u.next;
      a.next = u, u.prev = a, o.length -= f;
    }
    function k(o) {
      for (var a = [], l = o.head.next; l !== o.tail; )
        a.push(l.value), l = l.next;
      return a;
    }
    if (!r.document)
      return r.addEventListener && (c.disableWorkerMessageHandler || r.addEventListener("message", function(o) {
        var a = JSON.parse(o.data), l = a.language, u = a.code, f = a.immediateClose;
        r.postMessage(c.highlight(u, c.languages[l], l)), f && r.close();
      }, !1)), c;
    var m = c.util.currentScript();
    m && (c.filename = m.src, m.hasAttribute("data-manual") && (c.manual = !0));
    function y() {
      c.manual || c.highlightAll();
    }
    if (!c.manual) {
      var R = document.readyState;
      R === "loading" || R === "interactive" && m && m.defer ? document.addEventListener("DOMContentLoaded", y) : window.requestAnimationFrame ? window.requestAnimationFrame(y) : window.setTimeout(y, 16);
    }
    return c;
  }(e);
  n.exports && (n.exports = t), typeof _ < "u" && (_.Prism = t), t.languages.markup = {
    ...
    var r = "Loading…", s = function(m, y) {
      ...
      h: "c",
      tex: "latex"
    }, c = "data-src-status", d = "loading", P = "loaded", x = "failed", I = "pre[data-src]:not([" + c + '="' + P + '"]):not([' + c + '="' + d + '"])';
    function S(m, y, R) {
      var o = new XMLHttpRequest();
      o.open("GET", m, !0), o.onreadystatechange = function() {
        o.readyState == 4 && (o.status < 400 && o.responseText ? y(o.responseText) : o.status >= 400 ? R(s(o.status, o.statusText)) : R(i));
      }, o.send(null);
    }
    function j(m) {
      var y = /^\s*(\d+)\s*(?:(,)\s*(?:(\d+)\s*)?)?$/.exec(m || "");
      if (y) {
        var R = Number(y[1]), o = y[2], a = y[3];
        return o ? a ? [R, Number(a)] : [R, void 0] : [R, R];
      }
    }
    t.hooks.add("before-highlightall", function(m) {
      ...
      if (y.matches(I)) {
        m.code = "", y.setAttribute(c, d);
        var R = y.appendChild(document.createElement("CODE"));
        R.textContent = r;
        var o = y.getAttribute("data-src"), a = m.language;
        if (a === "none") {
          var l = (/\.(\w+)$/.exec(o) || [, "none"])[1];
          a = p[l] || l;
        }
        t.util.setLanguage(R, a), t.util.setLanguage(y, a);
        var u = t.plugins.autoloader;
        u && u.loadLanguages(a), S(
          o,
          function(f) {
            y.setAttribute(c, P);
            var A = j(y.getAttribute("data-range"));
            if (A) {
              var C = f.split(/\r\n?|\n/g), g = A[0], T = A[1] == null ? C.length : A[1];
              g < 0 && (g += C.length), g = Math.max(0, Math.min(g - 1, C.length)), T < 0 && (T += C.length), T = Math.max(0, Math.min(T, C.length)), f = C.slice(g, T).join(`
`), y.hasAttribute("data-start") || y.setAttribute("data-start", String(g + 1));
            }
            R.textContent = f, t.highlightElement(R);
          },
          function(f) {
            y.setAttribute(c, x), R.textContent = f;
          }
        );
      }
      ...
      highlight: function(y) {
        for (var R = (y || document).querySelectorAll(I), o = 0, a; a = R[o++]; )
...
class je {
  ...
  buildCodePanel() {
    ...
    `, x = (I) => {
      const S = e[I];
      c = S.source;
      const j = document.createElement("pre"), k = document.createElement("code");
      k.className = "language-cpp", k.textContent = S.source, j.appendChild(k), i.innerHTML = "", i.appendChild(j), xe.highlightElement(k);
    };
    p.addEventListener("click", async () => {
      try {
        await navigator.clipboard.writeText(c), p.innerHTML = P, p.classList.add("copied"), setTimeout(() => {
          p.innerHTML = d, p.classList.remove("copied");
        }, 1500);
      } catch (I) {
      ...
      j.className = "tab-button", j.textContent = I.name, S === 0 && j.classList.add("active"), j.addEventListener("click", () => {
        s.querySelectorAll(".tab-button").forEach((k) => k.classList.remove("active")), j.classList.add("active"), x(S);
      }), s.appendChild(j);
    }), this.codePanel.appendChild(s), this.codePanel.appendChild(p), this.codePanel.appendChild(i), e.length > 0 && x(0);
  }
}
function me(n, e) {
  switch (n) {
...
async function ye(n, e, t, r) {
  return `/demos/${n}/shadertoy.config.json` in t ? qe(n, t, e, r) : ke(n, e);
}
...
async function qe(n, e, t, r) {
  var R, o, a, l, u, f, A, C;
  const s = `/demos/${n}/shadertoy.config.json`, i = await e[s]();
  let p = null;
  if (i.common) {
  ...
  for (const g of d) {
    const T = i.passes[g];
    if (T)
      for (const q of ["iChannel0", "iChannel1", "iChannel2", "iChannel3"]) {
        const H = (R = T.channels) == null ? void 0 : R[q];
        H && "texture" in H && c.add(H.texture);
      }
  }
  const P = [], x = /* @__PURE__ */ new Map();
  for (const g of c) {
    const T = `/demos/${n}/${g.replace(/^\.\//, "")}`;
    if (!(T in r))
      throw new Error(`Texture not found: ${g} (expected at ${T})`);
    const q = await r[T](), H = g.split("/").pop().replace(/\.[^.]+$/, "");
    P.push({
      name: H,
      source: q,
      filter: "linear",
  ...
  for (const g of d) {
    ...
    }, H = T.source || q[g], w = `/demos/${n}/${H}`;
    if (!(w in t))
      throw new Error(`Missing shader file: ${w}`);
    const W = await t[w](), N = [
      L((o = T.channels) == null ? void 0 : o.iChannel0, x),
      L((a = T.channels) == null ? void 0 : a.iChannel1, x),
      L((l = T.channels) == null ? void 0 : l.iChannel2, x),
      L((u = T.channels) == null ? void 0 : u.iChannel3, x)
    ];
    I[g] = {
      name: g,
      glslSource: W,
  ...
  const S = ((f = i.meta) == null ? void 0 : f.title) || n.split("-").map((g) => g.charAt(0).toUpperCase() + g.slice(1)).join(" "), j = ((A = i.meta) == null ? void 0 : A.author) || null, k = ((C = i.meta) == null ? void 0 : C.description) || null, m = i.layout || "centered", y = i.controls ?? !1;
  return {
    root: `/demos/${n}`,
    meta: { title: S, author: j, description: k },
    layout: m,
    controls: y,
    commonSource: p,
    passes: I,
...
const Re = "demofox-pt2";
async function He() {
  return ye(Re, /* @__PURE__ */ Object.assign({
    "/demos/demofox-pt2/bufferA.glsl": () => Promise.resolve().then(() => Ve).then((r) => r.default),
    "/demos/demofox-pt2/common.glsl": () => Promise.resolve().then(() => be).then((r) => r.default),
    "/demos/demofox-pt2/image.glsl": () => Promise.resolve().then(() => ve).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/demofox-pt2/shadertoy.config.json": () => Promise.resolve().then(() => We).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/demofox-pt2/textures/equirect.jpg": () => Promise.resolve().then(() => Ge).then((r) => r.default)
  }));
}
async function Ee(n) {
  ...
  const t = await He(), r = me(t.layout, {
    container: e,
    project: t
  }), s = new Ie({
  ...
  return s.hasErrors() || s.start(), {
    ...
    destroy: () => {
      var i;
      (i = s.stop) == null || i.call(s);
    }
  };
}
const Oe = `uint wang_hash(inout uint seed)
{
...
`, Ve = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Oe
}, Symbol.toStringTag, { value: "Module" })), Ue = `// The minimunm distance a ray must travel before we consider an intersection.
...
`, be = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Ue
}, Symbol.toStringTag, { value: "Module" })), we = `void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
...
`, ve = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  title: "Path Tracing II",
  description: "From Demofox's Blog"
}, te = "fullscreen", ne = "common.glsl", re = {
  BufferA: {
    channels: {
      iChannel0: {
        buffer: "BufferA"
