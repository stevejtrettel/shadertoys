...
const float EQ_RADIUS = 4.0;

// Truncation: geodesics at radii e^(-a) and e^a
const float TRUNC_A = 0.2;
...
bool inStrip(vec2 z) {
    return distToVerticalGeodesic(z, EQ_CENTER) < EQ_RADIUS && z.x < EQ_CENTER;
}
...
vec3 drawStripRegion(vec2 z, vec3 baseColor) {
    ...
    // Edges along equidistant boundary
    if (inT && z.x < EQ_CENTER) {
    ...
    // Edge along central geodesic
    if (inT && distToVerticalGeodesic(z, EQ_CENTER) < EDGE_WIDTH && z.x < EQ_CENTER + 0.01) {
        color = REGION_EDGE;
    }
...
if (diskDist < DISK_RADIUS) {
    // Translate in disk before converting to UHP
    vec2 wShifted = diskTranslate(w, vec2(0.0, -0.7));  // positive moves strip down
    vec2 z = diskToUHP(wShifted);
