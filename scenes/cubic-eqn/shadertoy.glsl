...
vec2 f(float p, float q, vec2 z){

    vec2 cube = cmult(z,z,z);
    vec2 linear = 3.*p*z;
    vec2 constant = 2.*vec2(q,0);

    return cube+linear+constant;
}
...
vec3 gridColor(vec2 w){
    ...
    float grid = grid1+grid2+grid3+grid4;
    grid *=4.;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 w = f(cos(iTime),sin(iTime),z);
    //vec2 w = f(0.3,-0.3,z);
