...
vec3 getColor(vec2 fragCoord){
    ...
    Triangle T = Triangle(
    HalfSpace(Geodesic(-0.5,infty),1.),
    HalfSpace(Geodesic(0.5,infty),-1.),
    HalfSpace(Geodesic(-1.,1.),1.)
    );
    ...
    //color just the edge giving the infinty-gon
    g=T.c.bdy;
    ...
    //if(dist(w,vec2(0.5,1.) < 1.){
    // color = vec3(1,,0,0);
    //}

    float distToPoint = min(dist(w,vec2(0.5,sqrt(3.)/2.)),dist(w,vec2(-0.5,sqrt(3.)/2.)));
    if(distToPoint<0.15){
        color=0.75*darkBlue;
    }
    if(distToPoint<0.1){
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    col += getColor(fragCoord+vec2(0.5,0));
    col += getColor(fragCoord+vec2(-0.5,0));
    col += getColor(fragCoord+vec2(0,0.5));
    col += getColor(fragCoord+vec2(0,-0.5));
    col += getColor(fragCoord+vec2(0.5,0.5));
    col += getColor(fragCoord+vec2(-0.5,0.5));
    col += getColor(fragCoord+vec2(-0.5,0.5));
    col += getColor(fragCoord+vec2(-0.5,-0.5));
    col /= 9.;
